...
export class UsuariosListaPagina implements OnInit {
  ...
  usuarios: Usuario[] = [];
  cargando = false;
  columnas = ['nombre', 'apellido', 'correo', 'rol', 'acciones'];
  ...
  abrirFormularioNuevo(): void {
    ...
    this.formulario.get('contrasena')?.setValidators([Validators.required, Validators.minLength(6)]);
  ...
  guardar(): void {
    ...
    if (this.editando && this.usuarioSeleccionadoId) {
      this.puerto.actualizar(this.usuarioSeleccionadoId, datos).subscribe({
        next: () => {
  ...
  imprimirPDF(): void {
    window.open('http://localhost:9090/reportes/estudiantes/pdf?curso=General', '_blank');
  }
}